...
| OBJ-2 | Let downstream profiles express common section, table, ID, reference, text, and required-frontmatter policies without writing TypeScript validators. | First profile-backed validation exercise |
...
| REQ-4 | Functional | Must | The system shall support selectors for document, section, heading, table, table row, table cell, text span, link, and list targets. | Common profile policies need structural selection without custom traversal. `frontmatter` selector support is deferred from v1 because `frontmatterRequired` remains document-scoped and no frontmatter value predicates are included. | VAL-3 / VAL-4 |
...
| TECH-4 | Selector resolver | Selector layer | Resolve document, section, table, row, cell, text span, link, and list selections from public `EngineDocument` fields. | FUNC-3 |
...
- String comparisons use deterministic Unicode code point comparison over normalized `EngineDocument` text. Heading, section, header, column, `equals`, and `frontmatterRequired` field names match exactly. `includes`, `contains`, `excludes`, and `textOccurrenceCount.text` use non-overlapping literal substring matching. `textOccurrenceCount` requires the configured number of non-overlapping occurrences per selected target, including selected `tableCell` targets.
...
- ID occurrence identity is based on one normalized visible token, not on every structural projection that exposes that token. When a normalized document-text offset is available, it is the primary identity. Otherwise the engine falls back to the selected segment target and token offset, then to an aligned visible-source range. Markdown syntax, duplicated rich-IR projections, and source-less targets must not create additional occurrences for the same normalized token.
- Parser-provided visible text ranges are used for ID diagnostics when they can be aligned to the normalized token. Escaped visible link text such as `[REQ\-1](REQ-1.md)` maps to the link text source, not to matching destination URL syntax. IDs split across inline formatting boundaries such as `REQ-**1**` are supported through the same normalized visible-text identity.
- `references.idsFrom` collects source IDs using the same ID-token and occurrence-identity rules. When `idsFrom.column` is set, only cells in that column within the optional `idsFrom.section` contribute source IDs; other references in the same section do not create additional source IDs.
- `references.mustAppearIn` values are exact section titles. For each source ID and each listed target section, the target section body must contain at least one complete ID-token occurrence equal to that source ID. Matching is case-sensitive. If a source section is also a target section, the source occurrence itself does not satisfy the reference requirement, using the same occurrence identity as `ids.unique`. A missing reference diagnostic targets the source ID location when available and omits `sourceRange` when the source ID has no source evidence.
...
| `ids` | `document`, `section`, `heading`, `table`, `tableRow`, `tableCell`, `textSpan`, `link`, `list` | Collects ID tokens from selected target text. Use `tableCell` selectors for column-specific checks. |
...
| `text` | `document`, `section`, `heading`, `table`, `tableRow`, `tableCell`, `textSpan`, `link`, `list` | Evaluates literal text predicates against selected target text. Use `tableCell` selectors for column-specific checks. |
| `textOccurrenceCount` | `document`, `section`, `heading`, `table`, `tableRow`, `tableCell`, `textSpan`, `link`, `list` | Counts non-overlapping literal occurrences in selected target text. Use `tableCell` selectors for column-specific checks. |
| `frontmatterRequired` | `document` | Evaluates document frontmatter fields. `frontmatter` selectors are deferred from v1 and produce `profile.compile.unsupportedSelector`. |
...
| `profile.config.invalidShape` | `error` | A required field is missing, a field has the wrong type, a required array or string is empty, duplicate rule IDs are present, an invalid scalar value is supplied, a table predicate omits both `equals` and `includes`, or an assertion member contains no effective validation predicate. |
...
| VAL-2 | Test | Invalid syntax, duplicate rule IDs, unsupported keys, unsupported versions, unsupported selector targets, unsupported assertion members, incompatible selector/assertion pairs, invalid table predicates, ineffective assertion payloads, regex-like keys, and unsafe declarations produce the documented config or compile diagnostics and no compiled rule plan. | REQ-2 / REQ-8 / FUNC-1 / TECH-1 / TECH-2 / TECH-3 |
...
| VAL-4 | Test / Snapshot | Section, section-order, table header-array matching, table-predicate, ID-token, reference, literal-text, and document-scoped frontmatter assertions evaluate with the documented matching, cardinality, and source-targeting semantics against representative rich IR fixtures. | REQ-4 / REQ-5 / FUNC-3 / TECH-4 / TECH-5 |
...
| CR-9 | Major | Resolved | 14 / 17 | Consensus review found that ordered required-section validation did not define exact ordering, depth, subsequence, or duplicate-heading behavior. | Define `sectionsRequired.order: strict` as exact-title ordered subsequence matching over the normalized section tree flattened in source order, with deterministic duplicate handling and verification coverage. | Codex |
| CR-10 | Major | Resolved | 14 / 17 | Consensus review found that `ids` and `text` assertion payloads could be accepted as no-op assertion members. | Define effective assertion predicates for `ids` and `text`, classify no-op payloads as `profile.config.invalidShape`, and add verification coverage for ineffective assertion payloads. | Codex |
| CR-11 | Major | Resolved | 14 / 17 | Follow-up review found that duplicate declarative rule IDs would make public `ruleId` results, diagnostics, and evidence ambiguous. | Reject duplicate rule IDs as `profile.config.invalidShape`, explain the ambiguity rationale, update the diagnostic inventory, and add validation coverage. | Codex |
| CR-12 | Major | Resolved | 14 / 17 | Follow-up review found that `header` and `tableHeader` selector arrays did not define exact, subset, cardinality, or ordering behavior. | Define table header arrays as exact-title ordered subsequence matches over normalized table header rows, with deterministic duplicate handling and verification coverage. | Codex |
...
| Verification adequacy | 3 | Verification targets syntax parsing, duplicate rule IDs, unsupported declarations including regex-like keys, selector/assertion compatibility, table header-array matching, table predicates, reference semantics, closed compilation, source diagnostics, evidence hashes, repeatability, boundary audit, downstream exercise, and CLI behavior. |
