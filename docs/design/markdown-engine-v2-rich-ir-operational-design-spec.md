# Markdown Engine v2 Rich IR Operational Design Specification

## Document Control

| Field | Value |
| --- | --- |
| Title | Markdown Engine v2 Rich IR |
| Status | Draft |
| Rigor level | `R2` |
...
| Target milestone or release | `markdown-engine` v2 IR design approval |
...
Proposed outcome: A v2 IR contract that provides deterministic structural views, source-grounded spans, stable node targeting, table/list models, query helpers, and app-owned annotations while keeping domain semantics outside the engine.

Why now: `markdown-engine` v1 has proved parse, normalize, validate, serialize, deterministic output, and boundary safety; SpecTrace now shows the first concrete downstream pressure for richer generic structure before additional apps duplicate scanners.
...
| --- | --- | --- |
| OBJ-1 | Provide a generic rich IR that can support structural document apps without domain-specific parsing. | v2 implementation review |
| OBJ-2 | Provide stable source-grounded target identities for diagnostics, annotations, and downstream reports. | v2 implementation review |
...
| OBJ-4 | Reduce downstream line-scanner duplication by exposing sections, spans, tables, lists, source slices, and query helpers. | First SpecTrace or profile integration using v2 IR |
| NG-1 | This design will not implement SpecTrace entity registries, canonical IDs, relationship edges, or issue-key policy. | v2 implementation review |
| NG-2 | This design will not implement profile compilation, runtime lenses, MCP transport, agent adapters, or semantic validation. | v2 implementation review |
| NG-3 | This design will not introduce a graph database, persistent index, network service, or file-watching daemon. | v2 implementation review |
| NG-4 | This design will not promise stable node IDs across arbitrary content edits. | v2 implementation review |
...
| --- | --- | --- |
| Project owner | Approves v2 IR scope, compatibility policy, and implementation readiness. | Approve |
| Markdown-engine implementer | Implements IR, query, annotation, compatibility, and test changes. | Review |
| SpecTrace consumer | Confirms v2 generic IR can replace brittle Markdown line scanning where appropriate. | Review |
| Future profile/runtime consumer | Confirms v2 IR supports profile compilation and runtime lens generation without parser forking. | Review |
...
| CON-3 | Constraint | Public v2 IR changes are semver-classified before release or tag. | Existing API contract policy | Verify compatibility notes and v2 migration guidance in `VAL-7`. |
...
| ASM-1 | Assumption | Parser source positions are sufficient to produce source slices for the targeted GFM constructs. | v1 source ranges are already present for representative fixtures. | Validate through expanded source-slice fixtures in `VAL-3`. |
...
| REQ-9 | Compatibility | Must | The system shall preserve v1 parse, normalize, validate, and serialize behavior behind explicit v2 compatibility gates. | Existing consumers and evidence snapshots need controlled migration. | VAL-7 |
| REQ-10 | Reliability | Must | The system shall produce byte-for-byte identical serialized v2 output for identical input, options, package version, and runtime version. | v2 remains a deterministic review and CI substrate. | VAL-5 |
...
| REQ-12 | Operability | Must | The system shall document v2 IR fields, query helpers, annotation targets, migration behavior, and non-goals before release. | Durable consumers need a contract rather than source-code inference. | VAL-7 |
...
| --- | --- | --- | --- | --- |
| Section scanner replacement | SpecTrace BEL-905 scanner worktree contains line-based section extraction. | A SpecTrace-style fixture can identify section-owned references using v2 section/query APIs without raw line section scanning. | First v2 downstream exercise | OBJ-1 / OBJ-4 / REQ-2 / REQ-7 |
| Source target stability | v1 has source ranges but no public node target contract. | A 10-run repeatability test produces identical node targets and serialized v2 JSON for representative fixtures. | v2 implementation review | OBJ-2 / REQ-1 / REQ-10 |
| Table structural coverage | v1 has table nodes but no public row or cell coordinate contract. | Table fixture snapshots include cell text, row index, column index, and header state. | v2 implementation review | OBJ-1 / REQ-5 |
| Boundary preservation | v1 boundary audit reports no forbidden dependencies or semantic scope drift. | v2 boundary audit reports no SpecTrace/profile/runtime/MCP/agent/LLM/network-service direct dependency or domain-specific IR field. | v2 implementation review | OBJ-3 / REQ-8 / REQ-11 |
| Contract documentation | v1 has API/frontmatter contracts but no v2 rich IR guide. | v2 contract docs include migration notes, field definitions, query helpers, annotation target rules, and app non-goals. | v2 release readiness review | OBJ-1 / REQ-9 / REQ-12 |
...
| --- | --- | --- | --- | --- |
| v2 public package API | `markdown-engine` | Package consumers and CI | Markdown text, path, parse/normalize options, validation config | v2 parse, normalize, validate, query, annotation, and serialization results |
| v2 IR contract | `markdown-engine` | SpecTrace, profile/runtime, docs tools | Parsed Markdown and normalization options | Rich document tree, sections, spans, tables, lists, node targets, source references |
| Query helper API | `markdown-engine` | Downstream apps | v2 document and query options | Sections, nodes, spans, links, tables, lists, source slices |
...
| Parser adapter | `markdown-engine` | GFM parser dependency | Markdown body text and parser options | Parser output converted to engine-owned v2 IR |
| Serialization contract | `markdown-engine` | CI, agents, review tools | v2 public result objects | Stable JSON output |
...
| --- | --- | --- | --- | --- |
| FLOW-1 | A caller normalizes Markdown with v2 options. | Markdown text is supplied through the public API. | The caller receives rich IR with deterministic node targets, source ranges, sections, spans, tables, and lists. | REQ-1 / REQ-2 / REQ-3 / REQ-4 / REQ-5 / REQ-6 |
| FLOW-2 | A downstream app asks for the body of a heading section. | A v2 document contains headings. | The query helper returns the section node, child sections, and body node targets without raw line scanning. | REQ-2 / REQ-7 |
| FLOW-3 | A downstream app scans for domain tokens. | A v2 document contains normalized text spans with source ranges. | The app receives text spans and computes app-owned tokens outside core IR. | REQ-4 / REQ-7 / REQ-8 |
...
| FLOW-5 | A caller serializes v2 output for review evidence. | v2 normalization, validation, query, or annotation processing completed. | The caller receives deterministic JSON for identical input and options. | REQ-9 / REQ-10 |
| FUNC-1 | v2 normalization is invoked. | Input Markdown is parseable. | The API returns rich structural IR while preserving v1-compatible parse and validation behavior under documented gates. | REQ-1 / REQ-9 |
| FUNC-2 | Section query is invoked. | A v2 document has heading nodes. | The API returns section hierarchy and body membership derived from heading depth. | REQ-2 / REQ-7 |
| FUNC-3 | Text span query is invoked. | A v2 document has text-bearing nodes. | The API returns normalized text spans with node targets and source ranges. | REQ-3 / REQ-4 / REQ-7 |
| FUNC-4 | Table or list query is invoked. | A v2 document contains tables or lists. | The API returns normalized table/list structures with coordinates or item metadata. | REQ-5 / REQ-6 / REQ-7 |
...
States and transitions: The engine remains stateless across calls. Per invocation it transitions through InputReceived, Parsed, NormalizedV2, DerivedViewsBuilt, ConfigValidated, RulesEvaluated, AnnotationsChecked, and Serialized. Faults produce diagnostics inside the returned result and do not mutate persistent state.
...
| Fault-4 | v2 output differs across repeated identical runs. | Release readiness fails until ordering or identity generation is corrected. | REQ-1 / REQ-10 |
...
| --- | --- | --- | --- |
| ACC-1 | Normalize a representative execution-spec Markdown fixture with v2 enabled. | The result includes stable node targets, section hierarchy, source ranges, text spans, table/list structures where present, and no parse diagnostics. | REQ-1 / REQ-2 / REQ-3 / REQ-4 / REQ-5 / REQ-6 / FUNC-1 |
...
| ACC-6 | Run v2 serialization 10 times over the same fixture and options. | All serialized bytes are identical across the 10 runs. | REQ-1 / REQ-10 / FUNC-1 / FUNC-5 |
| ACC-7 | Review v2 docs and migration guidance before release. | The contract docs identify fields, query helpers, annotation targets, compatibility gates, and non-goals. | REQ-9 / REQ-12 |
...
| --- | --- | --- | --- |
| REQ-1 | FLOW-1 / FLOW-5 / FUNC-1 | ACC-1 / ACC-6 | Node targeting is observable in v2 IR and serialization. |
...
| REQ-6 | FLOW-1 / FUNC-4 | ACC-1 | List structure is externally observable in v2 IR. |
...
| --- | --- | --- | --- | --- |
| TECH-1 | v2 document contract | Public API and IR types | Define public fields for node targets, sections, source references, spans, tables, lists, and annotations. | FUNC-1 |
...
| TECH-9 | Compatibility and serializer controls | Serialization and contract layer | Keep root APIs v1 by default, require an `irVersion: "2"` selector for v2 output, allow any `v2` namespace only as a wrapper around that selector, reject version mismatches, and serialize deterministic public results. | FUNC-1 / FUNC-5 |
...
Provisional v2 public shape:

```ts
interface EngineDocumentV2 {
  kind: "markdown-document";
  version: "2.0.0";
  path?: string;
  frontmatter?: unknown;
  children: EngineNodeV2[];
...
migration notes before v2 approval.
...
Compatibility gate decision: Existing root package APIs remain v1 by default.
Calling `parse`, `normalize`, `validate`, or `serialize` without
`irVersion: "2"` shall not return v2 document shapes or v2 serialized output.
The canonical v2 selector is an explicit `irVersion: "2"` option accepted by the
relevant API. The implementation may also provide a versioned `v2` export
namespace during migration, but that namespace must wrap the same option-gated
implementation and must not create a second compatibility policy. A v2 document
must carry `version: "2.0.0"`, and v2 validation or serialization must reject
or diagnose a mismatch between the requested IR version and the document
version. Migration documentation in `VAL-7` shall list the exact v1-default
entry points, v2 entry points, option names, and semver classification before
release approval.

| Change | Type | Compatibility impact | Reversibility | Mitigation |
| --- | --- | --- | --- | --- |
| v2 `EngineDocument` contract | Schema | Adds public rich IR fields and may require a versioned document shape for consumers. | Reversible before v2 release; semver-controlled after release | Version the document contract and preserve v1 gates until migration is approved. |
| Node target format | Schema | New public target identifier used by diagnostics and annotations. | Reversible before v2 release; semver-controlled after release | Document target stability limits and snapshot representative fixtures. |
| Section, span, table, and list views | Schema | New derived views become public downstream contracts. | Reversible before v2 release; semver-controlled after release | Treat as v2-only contract with compatibility docs and fixture snapshots. |
| Query helper API | API | New package functions or namespaces for structural access. | Reversible before v2 release; semver-controlled after release | Add typed API tests and docs before release. |
| Annotation target contract | API / Schema | New app-owned extension surface for domain annotations. | Reversible before v2 release; semver-controlled after release | Keep annotation payload opaque and validate only target shape. |
| Serialization output | Data | v2 output changes snapshots and downstream review evidence. | Reversible before v2 release; semver-controlled after release | Require explicit v2 serialization mode and repeatability evidence. |

Section status: Complete

## 15. Control Logic and Non-Functional Controls

Control logic summary: v2 normalization first creates the engine-owned node tree, then derives node targets, source-map entries, section hierarchy, text spans, table/list views, query indexes, and optional annotation attachment results in deterministic order. Validation rules continue to operate on normalized public IR and do not gain semantic execution.
...
Failure recovery model: Missing parser offsets, malformed annotation targets, unsupported v2 options, or incompatible serialization requests produce diagnostics or typed errors documented by the public API. No persistent recovery action is required because the engine mutates no external state.
...
| REQ-9 | TECH-1 / TECH-9 | Versioning and compatibility gates protect v1 consumers. |
...
| --- | --- | --- | --- |
| v2 fixture test output | Log | Show that rich IR fixtures pass and snapshots are stable. | Implementer and reviewer |
| v2 repeatability output | Log | Show that node targets and serialized output are stable across repeated runs. | Reviewer |
...
| Contract review checklist | Audit | Show that v2 IR, query helpers, annotations, and migration notes are documented. | Project owner and downstream consumers |
| SpecTrace-style fixture exercise | Log | Show whether v2 IR supports section and span use cases that motivated the design. | SpecTrace consumer |

Rollout plan: Implement v2 behind explicit document version and option gates: root APIs remain v1 by default, and v2 output requires an `irVersion: "2"` selector. A versioned `v2` namespace may wrap the same selector during migration but may not create a separate compatibility policy. Add representative fixture snapshots; update contract docs; run v1 and v2 test suites together; run a SpecTrace-style fixture exercise; request downstream review before any package tag or publication.

Rollback or containment plan: Trigger rollback if v1 behavior changes without explicit approval, v2 repeatability fails, boundary audit detects domain leakage, or downstream review rejects the target model. The rollback action is to withhold v2 release and revert v2 contract changes on the branch; containment limit is source/package contract state because no persistent user data exists.

Operator actions: Run build, typecheck, root test suite, v2 targeted tests, repeatability script, boundary audit, contract review checklist, and SpecTrace-style fixture exercise before requesting approval.
...
| --- | --- | --- | --- |
| VAL-1 | Test | v1 parse, normalize, validate, and serialize behavior remains unchanged when root APIs are called without a v2 selector. | REQ-9 / TECH-9 |
...
| VAL-5 | Test | Ten repeated v2 runs produce byte-for-byte identical serialized output, node targets, derived views, and annotation attachment results. | REQ-1 / REQ-10 / TECH-2 / TECH-9 / TECH-10 |
...
| VAL-7 | Review | v2 contract docs cover fields, helpers, annotations, migration behavior, exact v1-default and v2-gated entry points, compatibility classification, and non-goals. | REQ-9 / REQ-12 / TECH-1 / TECH-9 |
...
| VAL-9 | Test / Downstream exercise | A SpecTrace-style fixture can use v2 sections, spans, and annotation targets while keeping entity registry semantics outside the engine. | REQ-2 / REQ-4 / REQ-8 / FUNC-2 / FUNC-3 / FUNC-5 / TECH-3 / TECH-5 / TECH-8 |
...
| --- | --- | --- |
| Keep v1 IR and let downstream apps scan raw Markdown | Lowest engine churn and fastest downstream experimentation. | Duplicates section, span, source, and table logic in every app and weakens the engine as the deterministic substrate. |
...
| --- | --- | --- | --- | --- |
| RISK-1 | v2 IR may become too broad and absorb app semantics. | Medium | High | Enforce `CON-1`, `CON-2`, `VAL-8`, and annotation payload opacity. |
...
| RISK-5 | v2 compatibility gates may leave v1 and v2 behavior ambiguous for consumers. | Medium | Medium | Require migration docs, explicit options, and v1/v2 tests in `VAL-1` and `VAL-7`. |
...
| --- | --- | --- | --- | --- | --- | --- |
| ST-1 | Major | Resolved | 14 | The initial draft named v2 schema surfaces but did not provide enough provisional public shape detail for `R2` implementation review. | Add provisional v2 public shape definitions and require `VAL-7` to freeze names before release approval. | Codex |
...
| EXT-2 | Major | Resolved | 14 / 16 / 17 | External review found that the v2 compatibility gate was underspecified. | Record that root APIs remain v1 by default and v2 requires an `irVersion: "2"` selector, with any `v2` namespace limited to a wrapper, plus mismatch checks and documentation gates. | Codex |
...
| SM-1 | Observation | Closed | 18 | The design correctly excludes SpecTrace semantics from the engine but depends on a downstream exercise to prove the annotation boundary is usable. | Keep `VAL-9` as a required downstream exercise before v2 release approval. | Project owner |

Semantic scores:

| Dimension | Score | Notes |
| --- | --- | --- |
| Problem validity | 3 | Grounded in current v1 IR and SpecTrace scanner pressure. |
...
| Non-functional adequacy | 3 | Determinism, compatibility, default-v1 behavior, and boundary controls are explicit. |
